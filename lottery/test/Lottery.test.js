...
describe("Lottery Contract",() => {
    ...
    it('pick winner is only called by manager',async () => {
        try{
    ...
    it('sends money to the winner and resets the player array',async () => {
        ...
        // TODO: assert that player array gets empty
        const players = await lottery.methods.getPlayers().call();

        assert(players.length == 0);

        // TODO: assert that total money in contract is zero

        var addressValue = await lottery.options.address;
        const value = await web3.eth.getBalance(addressValue);
        assert(value == 0);
    });
